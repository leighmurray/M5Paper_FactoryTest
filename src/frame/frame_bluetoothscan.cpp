...
Frame_BluetoothScan::Frame_BluetoothScan(void) {
    ...
    for (int i = 0; i < BT_MAX_DEVICES + 1; i++) {
        _key_device[i] = new EPDGUI_Button(4, 100 + i * 60, 532, 61);
    ...
    _canvas_status->createCanvas(532, 60);
    ...
    if (_language == LANGUAGE_JA) {
        exitbtn("ホーム");
        _canvas_title->drawString("BTキーボード", 270, 34);
    } else if (_language == LANGUAGE_ZH) {
        exitbtn("主页");
        _canvas_title->drawString("蓝牙键盘", 270, 34);
    } else {
        exitbtn("Home");
        _canvas_title->drawString("BT Keyboard", 270, 34);
    }
...
void Frame_BluetoothScan::UpdateDeviceList(int count) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        DrawDevice(_key_device[i], dev->name[0] ? dev->name : dev->address);
    ...
    _key_device[ri]->CanvasNormal()->drawRect(0, 0, 532, 61, 15);
...
int Frame_BluetoothScan::init(epdgui_args_vector_t &args) {
    ...
    // Show "Scanning..." and kick off scan
    _canvas_status->fillCanvas(0);
    if (_language == LANGUAGE_ZH || _language == LANGUAGE_JA) {
...
int Frame_BluetoothScan::run() {
    ...
    if (_connecting && ble_is_connected()) {
        _connecting = false;
        _canvas_status->fillCanvas(0);
        if (_language == LANGUAGE_ZH || _language == LANGUAGE_JA) {
            _canvas_status->drawString("已连接!", 266, 30);
        } else {
    ...
    if (_scanning && g_ble_scan_result >= 0) {
        _scanning = false;
        UpdateDeviceList(g_ble_scan_result);
        g_ble_scan_result = -1;
    }
