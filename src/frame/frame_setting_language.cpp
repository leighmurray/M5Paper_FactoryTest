...
Frame_Setting_Language::Frame_Setting_Language(void) {
    ...
    _sw_en = new EPDGUI_Switch(2, 4, 100, 952, 61);
    _sw_zh = new EPDGUI_Switch(2, 4, 160, 952, 61);
    _sw_ja = new EPDGUI_Switch(2, 4, 220, 952, 61);
    ...
    if (language == LANGUAGE_JA) {
        exitbtn("設定");
        _canvas_title->drawString("言語", 480, 34);
        _sw_ja->setState(1);
    } else if (language == LANGUAGE_ZH) {
        exitbtn("设置");
        _canvas_title->drawString("语言", 480, 34);
        _sw_zh->setState(1);
    } else {
        exitbtn("Setting");
        _canvas_title->drawString("Language", 480, 34);
        _sw_en->setState(1);
    }
