...
void key_btscan_cb(epdgui_args_vector_t &args) {
    Frame_Base *frame = EPDGUI_GetFrame("Frame_BluetoothScan");
    if (frame == NULL) {
        frame = new Frame_BluetoothScan();
        EPDGUI_AddFrame("Frame_BluetoothScan", frame);
    }
    EPDGUI_PushFrame(frame);
    *((int *)(args[0])) = 0;
}
...
Frame_Main::Frame_Main(void) : Frame_Base(false) {
    ...
    *(_key[kKeyHome]->CanvasPressed()) = *(_key[kKeyHome]->CanvasNormal());
    ...
    _key[kKeyBTScan]->CanvasNormal()->pushImage(
        0, 0, 92, 92, ImageResource_main_icon_bluetooth_92x92);
    *(_key[kKeyBTScan]->CanvasPressed()) = *(_key[kKeyBTScan]->CanvasNormal());
    _key[kKeyBTScan]->CanvasPressed()->ReverseColor();
    _key[kKeyBTScan]->AddArgs(EPDGUI_Button::EVENT_RELEASED, 0,
                              (void *)(&_is_run));
    _key[kKeyBTScan]->Bind(EPDGUI_Button::EVENT_RELEASED, key_btscan_cb);
