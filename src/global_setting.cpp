...
static char global_ble_name[64]    = {0};
...
esp_err_t LoadSetting(void) {
    ...
    char ble_name_buf[64] = {0};
    size_t ble_name_len = sizeof(ble_name_buf);
    if (nvs_get_str(nvs_arg, "ble_name", ble_name_buf, &ble_name_len) == ESP_OK)
        strncpy(global_ble_name, ble_name_buf, sizeof(global_ble_name) - 1);
...
void SetBLEDeviceName(const char *name) {
    strncpy(global_ble_name, name ? name : "", sizeof(global_ble_name) - 1);
    nvs_handle_t handle;
    if (nvs_open("Setting", NVS_READWRITE, &handle) == ESP_OK) {
        nvs_set_str(handle, "ble_name", global_ble_name);
        nvs_commit(handle);
        nvs_close(handle);
    }
}
...
const char *GetBLEDeviceName(void) { return global_ble_name; }
